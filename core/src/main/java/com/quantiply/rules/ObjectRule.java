...
import com.quantiply.util.ParcelableHelper;
import com.quantiply.util.ReflectionHelper;
import com.quantiply.util.SerializableHelper;
import com.quantiply.util.TypeUtil;
...
import com.quantiply.exception.ClassAlreadyExistsException;
        ...
        try {
            if (node.has("existingJavaType")) {
                ...
                if (PrimitiveTypes.isPrimitive(fqn, _package.owner())) {
                    throw new ClassAlreadyExistsException(PrimitiveTypes.primitiveType(fqn, _package.owner()));
                }

                JClass existingClass = TypeUtil.resolveType(_package, fqn + (node.get("existingJavaType").asText().contains("<") ? "<" + substringAfter(node.get("existingJavaType").asText(), "<") : ""));
